...
router.post('/', optionalAuth, [
  ...
  try {
    const { message, sessionId: providedSessionId, useTemplate } = req.body;
    ...
    // Check if user wants quick template response
    if (useTemplate === true) {
      ...
      return res.json({ reply: templateReply, sessionId, source: 'template' });
    }

    // Try template response first (instant)
    const templateReply = findTemplateResponse(message);
    ...
    // Try OpenRouter, fallback to template
    let reply = null;
    let errorMsg = null;
    let source = 'ai';
    ...
    res.json({ reply, sessionId, source });
  } catch (error) {
